...
$(() => {
    ...
    modal.inject(modal.type.alert, 'search').then(modal_obj => {
        ...
        if (args.has('keyword')) {
            ...
            document.title = filters.keyword.charAt(0).toUpperCase() + filters.keyword.slice(1);

            if (filters.keyword === 'favourites') {
                $('.keyword .name').html('Our favourites');
            }
            else if (filters.keyword === 'bestsellers') {
                $('.keyword .name').html('Bestsellers');
            }
            else if (filters.keyword === 'latest') {
                $('.keyword .name').html('What\'s new');
                $(".filter.orderby option[value='year']").remove();
            }

            $('.filter.orderby').val(filters.orderby);
            $('.search-form .keyword').val(filters.keyword);
            ...
            let query_args = {
                extra: filters.keyword,
                orderby : filters.orderby,
                offset : filters.page
            };

            if (query_args.keyword === 'latest' && query_args.orderby === 'year') {
            ...
            loading.hide();
            modal.show('Warning', 'No keyword was provided!')
        }
    })
});