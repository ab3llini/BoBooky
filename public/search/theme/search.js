...
$(() => {
    ...
    modal.inject(modal.type.alert, 'search').then(modal_obj => {
        ...
        if (args.has('theme')) {
            ...
            document.title = filters.theme.charAt(0).toUpperCase() + filters.theme.slice(1);
            $('.theme-ribbon .name').html(filters.theme);
            $('.search-form .theme').val(filters.theme);
            ...
            loading.hide();
            modal.show('Warning', 'No theme was provided!')
        }
    })
});